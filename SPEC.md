...
Семейный выбор имени для ребёнка. Каждый участник голосует тайно, результаты открываются когда все закончили.
...
Всё приложение — один файл `index.html` (~1500 строк). CSS, JS и HTML вперемешку, без сборки.
...
|---|---|---|
| `spaces` | `id` | Голосование: id, name, creatorUid, status, joinedUids, _progress, _memberCount, _avgProgress |
| `votes` | `[spaceId, name]` | Голос: score (1–4) |
| `outbox` | autoIncrement | Задание для синхронизации: type, payload |
| `names` | `name` | Имя из датасета |
